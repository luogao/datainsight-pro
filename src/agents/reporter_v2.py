...
@tool
def compile_summary(data_explorer_result: str = "", analyst_result: str = "", pandaai_result: str = "", **kwargs) -> str:
    """
...
> 生成时间：{timestamp}
> 分析系统：DataInsight Pro v2.0 (PandaAI + CrewAI)
...
@tool
def format_report_markdown(summary: str, **kwargs) -> str:
    """
    格式化报告为 Markdown 格式

    Args:
        summary: 摘要内容
        **kwargs: 其他可选参数（如 charts 列表）
...
@tool
def format_report_json(summary: str = "", metrics: dict = None, findings: list = None, recommendations: list = None, **kwargs) -> str:
    """
    格式化报告为 JSON 格式
    ...
    report_json = {
        ...
        "version": "2.0.0",
        "system": "DataInsight Pro (PandaAI + CrewAI)",
        "summary": summary,
        "metrics": metrics,
        "findings": findings,
...
@tool
def save_report(report_content: str, output_path: str = "report.md", format: str = "markdown", **kwargs) -> str:
    """
    保存报告到文件

    Args:
        report_content: 报告内容
        output_path: 输出路径（默认 report.md）
        format: 文件格式（markdown/json，默认 markdown）
        **kwargs: 其他可选参数
...
@tool
def generate_executable_summary(analysis_goal: str = "", key_findings: list = None, priority_actions: list = None, **kwargs) -> str:
    """
    生成可执行的执行摘要（供决策者使用）
